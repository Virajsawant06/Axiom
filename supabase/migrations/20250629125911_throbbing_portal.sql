/*
  ...
  1. New Columns
    - Add MMR tracking columns to users table
    - github_repos_count, hackathons_participated, etc.

  2. Functions
    - calculate_user_mmr() - Calculate MMR based on stats
    - update_user_mmr() - Update user's ranking
    - trigger_update_user_mmr() - Auto-update trigger

  3. Indexes
    - Performance indexes for search functionality

  4. Sample Data
    - Populate existing users with sample MMR data
*/
...
-- Drop existing trigger first (if it exists) to avoid dependency issues
...
-- Create trigger to automatically update MMR when stats change (fixed variable naming)
